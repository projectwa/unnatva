    ...
    <div class="container-xxl py-6" id="whats-happening">
        <div class="container py-0 px-lg-5">
            ...
            <div class="owl-carousel happening-carousel wow fadeInUp" data-wow-delay="0.1s">
                <?php 
                // In CodeIgniter, view data array keys become variables
                // So $whatsHappeningBlocks should be available directly
                // But let's ensure it exists and handle edge cases
                
                // Check if variable exists, if not, set to empty array
                if (!isset($whatsHappeningBlocks) || is_null($whatsHappeningBlocks)) {
                    $whatsHappeningBlocks = [];
                }
                
                // Ensure it's iterable (array or object with countable interface)
                if (!is_array($whatsHappeningBlocks) && !is_iterable($whatsHappeningBlocks)) {
                    $whatsHappeningBlocks = [];
                }
                
                // Debug: Log what we have
                $blockCount = is_countable($whatsHappeningBlocks) ? count($whatsHappeningBlocks) : 0;
                log_message('error', 'Home view: whatsHappeningBlocks count: ' . $blockCount . ', type: ' . gettype($whatsHappeningBlocks));
                
                if (empty($whatsHappeningBlocks) || $blockCount === 0): 
                    ...
                    <?php foreach ($whatsHappeningBlocks as $block): ?>
                        <div class="testimonial-item p-3 position-relative">
                            ...
                            <?php if (!empty($block->content)): ?>
                                <p class="small <?= empty($block->image) ? 'mb-3' : '' ?>"><?= nl2br(htmlspecialchars($block->content)) ?></p>
                            <?php endif; ?>
                            ...
                            <?php elseif (!empty($block->external_link)): ?>
                                <a href="<?= htmlspecialchars($block->external_link) ?>" 
                                   <?= (strpos($block->external_link, 'http') === 0) ? 'target="_blank"' : '' ?> 
                                   class="rounded-btn <?= empty($block->image) ? '' : 'm-3 position-absolute bottom-0 start-0' ?>">
                                    Read More <span class="arrow"><i class="bi bi-arrow-right"></i></span>
                                </a>
                            <?php endif; ?>
                            
                            <?php if (empty($block->image) && !empty($block->external_link)): ?>
                                <div class="d-flex align-items-center mt-4 position-relative">
                                    <!-- Image can be placed here if needed for layout without top image -->
                                </div>
                            <?php endif; ?>
                        </div>
                    <?php endforeach; ?>
